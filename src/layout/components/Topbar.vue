<template>
  <div class="top-nav">
    ...
    <el-menu :default-active="activeMenu" mode="horizontal" :active-text-color="variables.menuActiveText" @select="handleSelect">
      <div v-for="item in routes" :key="item.path" class="nav-item">
        <div v-if="isOnlyOneChild(item)">
          <app-link :to="resolvePath(item)">
            <el-menu-item v-if="!item.hidden" :index="item.path">{{ item.meta ? item.meta.title : item.children[0].meta.title }}
            </el-menu-item>
          </app-link>
        </div>
        <div v-else>
